...
import com.example.myapplication.snappadapter.layoutmanager.*
import kotlin.math.abs
import kotlin.math.min

internal class LoopingLayoutManager : RecyclerView.LayoutManager,
    ...
    @ScrollDirectionStrategy
    private var scrollDirectionStrategy: Int
    ...
    constructor(
        ...
        @ScrollDirectionStrategy scrollDirectionStrategy: Int = TOWARDS_UNDIFINED
    ) {
        this.orientation = orientation
        this.reverseLayout = reverseLayout
        this.scrollDirectionStrategy = scrollDirectionStrategy
    }
    ...
    constructor(context: Context, attrs: AttributeSet, defStyleAttr: Int, defStyleRes: Int) {
        ...
        this.scrollDirectionStrategy = TOWARDS_UNDIFINED
    }
    ...
    private fun scrollBy(
        ...
        var selectedItem = try {
            getInitialItem(movementDir)
        } catch (e: Throwable) {
            return 0
        }
    ...
    private fun shouldScroll(state: RecyclerView.State): Boolean {
        return state.itemCount > 1 || findFirstCompletelyVisibleItemPosition() == Int.MAX_VALUE
    }
    ...
    private fun computeScrollVectorForPosition(targetPosition: Int, count: Int): PointF {
        val movementDir = when {
            scrollDirectionStrategy == TOWARDS_HIGHER_INDICES && itemCount == 2 && targetPosition == 0 -> {
                addViewsAtOptAnchorEdge(targetPosition, this, count)
            }
            scrollDirectionStrategy == TOWARDS_LOWER_INDICES && itemCount == 2 && targetPosition == count - 1 -> {
                addViewsAtAnchorEdge(targetPosition, this, count)
            }
            else -> {
                smoothScrollDirectionDecider(targetPosition, this, count)
            }
        }
        return if (orientation == HORIZONTAL) {
    ...
    annotation class ScrollDirectionStrategy
